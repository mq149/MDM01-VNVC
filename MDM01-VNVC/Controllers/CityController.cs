...
namespace MDM01_VNVC.Controllers
{
    ...
    public class CityController : ControllerBase, IDisposable
    {
        ...
        public CityController(IOptions<GraphDatabaseSettings> settings)
        {
        ...
        public List<string> Get()
        {
            ...
            {
                var statement = $"match (n:TinhThanh) return n.tenTinhThanh as city order by city;";
                var result = session.Run(statement);
                
                foreach(var r in result)
                {
                    string i=r["city"].As<string>();
                    a.Add(i);
                }
            }
            _driver.CloseAsync();
